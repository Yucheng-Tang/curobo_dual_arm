...
from curobo.opt.newton.lbfgs import LBFGSOptConfig
...
class NullspaceOptBase(Optimizer, LBFGSOptConfig):
    @profiler.record_function("nullspace_opt/init")
    def __init__(
        self,
        config: Optional[LBFGSOptConfig] = None,
    ):
    ...
    def _optimize(self, q: T_BHDOF_float, shift_steps=0, n_iters=None, robot_jac: TorchJacobian=None, q_init: torch.Tensor=None):
        log_info("using nullspace optimizer!")
        ...
        with profiler.record_function("nullspace_base/init_opt"):
    ...
    # TODO: modify this function to use nullspace optimizer
    def _opt_step(self, q, grad_q, robot_jac, q_init):
        with profiler.record_function("nullspace/line_search"):
        ...
        with profiler.record_function("nullspace/step_direction"):
            grad_q = self._get_step_direction(cost_n, q_n, grad_q_n)
            # print("getting step direciton!!!", grad_q)
        with profiler.record_function("nullspace/update_best"):
